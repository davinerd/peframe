...
def show_signs(filename):
    ...
    sign_cert['issuer'] = ast.literal_eval(auth.cert_chain_head[2][0])
    ...
    for (issuer, serial), cert in auth.certificates.items():
        ...
        certz['serial'] = serial
        certz['subject'] = ast.literal_eval(str(dn.DistinguishedName.TraverseRdn(cert[0][0]['subject'][0])))
        ...
        certz['time_not_before'] = time.asctime(time.gmtime(not_before_time))
        certz['time_not_after'] = time.asctime(time.gmtime(not_after_time))
        bin_cert = der_encoder.encode(cert)
        certz['md5'] = hashlib.md5(bin_cert).hexdigest()
        certz['sha1'] = hashlib.sha1(bin_cert).hexdigest()
        std_output.print_sign(certz)
